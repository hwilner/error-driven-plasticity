...
4. **A public-boundary scanner.** `tools/check_public_boundary.py` lists tracked files via `git ls-files`, flags prohibited path names (for example `results/`, `data/`, `figures/`) and prohibited file suffixes (for example `.csv`, `.png`, `.ipynb`), and scans approved text files for contact patterns, reserved identifier patterns, unsupported scope claims, and numeric outcome claims. Tests cover one blocked path and one allowed path.
